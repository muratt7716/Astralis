...
export default function TarotPage() {
  ...
  const positions = spread === "three"
    ...
    : spread === "celtic"
    ? ["Durum", "Engel", "Bilinçaltı", "Geçmiş", "Olasılık", "Yakın Gelecek", "Tutum", "Çevre", "Umutlar", "Sonuç"]
    : ["Mesajınız"];
      ...
      <section className="pt-16 pb-8 px-4">
        <div className="max-w-4xl mx-auto text-center">
          ...
          <h1 className="text-4xl md:text-5xl font-bold mb-4">
            <span className="gradient-text">Tarot Falı</span>
          </h1>
          <p className="text-gray-400 text-lg max-w-2xl mx-auto">
            78 kartlık kadim kehanet destesinden kartlarınızı seçin. Her kart, hayatınıza dair derin bir mesaj taşır.
          </p>
        </div>
      </section>
        ...
        <section className="pb-6 px-4">
          <div className="max-w-3xl mx-auto space-y-4">
            <div className="glass-card p-6">
              ...
              <p className="text-gray-500 text-xs mt-2">{selectedSpread.description}</p>
            </div>
            <div className="glass-card p-6">
              <label className="block text-gray-400 text-xs uppercase tracking-wider mb-2">Sorunuz / Niyetiniz (İsteğe Bağlı)</label>
              <textarea value={question} onChange={e => setQuestion(e.target.value)} placeholder="Aklınızdaki bir soru veya niyet yazın..."
        ...
        <section className="pb-8 px-4 fade-in-up">
          <div className="max-w-5xl mx-auto">
            <div className="text-center mb-6">
              ...
              <p className="text-gray-400 text-sm">İçinizden gelen kartlara dokunun. Sezgileriniz sizi doğru kartlara yönlendirecek.</p>
            </div>
      ...
      {phase === "reading" && (
        <section className="pb-8 px-4 fade-in-up">
          <div className="max-w-4xl mx-auto">
            ...
            <div className={`grid gap-4 ${selectedCards.length <= 3 ? "grid-cols-1 sm:grid-cols-3 max-w-2xl mx-auto" : "grid-cols-2 md:grid-cols-5"}`}>
              {selectedCards.map((card, idx) => {
                ...
                return (
                  <div key={idx} className={`rounded-xl border text-center bg-gradient-to-b ${card.isReversed ? "from-red-900/30 to-gray-900/40 border-red-500/30" : colorClass} overflow-hidden fade-in-up`}
                    ...
                    <div className="p-3">
                      ...
                      {card.isReversed && <p className="text-red-400 text-xs font-medium">↻ Ters</p>}
                      <p className="text-gray-400 text-xs mt-1 leading-relaxed">
                        {card.isReversed ? card.reversedMeaning : card.upright}
                      </p>
                    </div>
                  </div>
                );
